...
const config:Config={
  ...
  customFields:{projects:parse(fs.readFileSync('sources.yml','utf8')).projects.map(({id,name,repository,targetPath}:Record<string,string>)=>({id,name,repository,targetPath}))},
