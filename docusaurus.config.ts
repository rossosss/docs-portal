...
const config:Config={
  ...
  customFields:{docsApiUrl:process.env.DOCS_API_URL || 'http://127.0.0.1:4000',projects:parse(fs.readFileSync('sources.yml','utf8')).projects.map(({id,name,repository,targetPath}:Record<string,string>)=>({id,name,repository,targetPath}))},
