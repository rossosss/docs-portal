...
Публикация: https://rossosss.github.io/docs-portal/ . Исходные проекты находятся в аккаунте rossosss.
Без отдельного DOCS_PORTAL_TOKEN портал автоматически проверяет SHA источников раз в 15 минут и пересобирается только при изменениях. Расписание GitHub может выполняться с задержкой. Для немедленного обновления используйте workflow_dispatch; dedicated dispatch PAT включает прямые уведомления из проектных workflows.
Если DOCS_API_URL не задан при production-сборке, веб-формы явно показывают, что Docs API ещё не подключён. Backend необходимо разместить отдельно на HTTPS-хостинге Node.js.
